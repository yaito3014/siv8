...
namespace s3d
{
	...
	StringView JPEGDecoder::name() const noexcept
	{
		return U"JPEG"_sv;
	}
	...
	std::span<const StringView> JPEGDecoder::possibleExtensions() const noexcept
	{
	...
	Optional<ImageInfo> JPEGDecoder::getImageInfo(const IReader& reader, const FilePathView) const
	{
			...
			|| ((buf[0] == 0xFF) && (buf[1] == 0xD8) && (buf[2] == 0xFF) && (buf[3] == 0xE1) && (buf[6] == 'E') && (buf[7] == 'x') && (buf[8] == 'i') && (buf[9] == 'f')))
		{
			...
			while (buf[2] == 0xFF)
			{
				...
				pos += (2LL + (static_cast<int64>(buf[4]) << 8) + buf[5]);
