...
namespace s3d
{
	...
	namespace detail
	{
		...
		{
			return (::stat(FilePath(path).replaced(U'\\', U'/').toUTF8().c_str(), &s) == 0);
		}
			...
			const static std::array<FilePath, 11> g_specialFolderPaths = []()
				{
					...
					specialFolderPaths[FromEnum(SpecialFolder::Desktop)] = AddSlash(EnvironmentVariable::Get(U"XDG_DESKTOP_DIR").removed(U'\"').replaced(U"$HOME", homeDirectory));
					specialFolderPaths[FromEnum(SpecialFolder::Documents)] = AddSlash(EnvironmentVariable::Get(U"XDG_DOCUMENTS_DIR").removed(U'\"').replaced(U"$HOME", homeDirectory));
					specialFolderPaths[FromEnum(SpecialFolder::Music)] = AddSlash(EnvironmentVariable::Get(U"XDG_MUSIC_DIR").removed(U'\"').replaced(U"$HOME", homeDirectory));
					specialFolderPaths[FromEnum(SpecialFolder::Pictures)] = AddSlash(EnvironmentVariable::Get(U"XDG_PICTURES_DIR").removed(U'\"').replaced(U"$HOME", homeDirectory));
					specialFolderPaths[FromEnum(SpecialFolder::Videos)] = AddSlash(EnvironmentVariable::Get(U"XDG_VIDEOS_DIR").removed(U'\"').replaced(U"$HOME", homeDirectory));
	...
	namespace FileSystem
	{
		...
		Array<FilePath> DirectoryContents(const FilePathView path, const Recursive recursive)
		{
			...
			if (recursive)
			{
				for (const auto& v : fs::recursive_directory_iterator(path.toUTF8()))
				{
			...
			{
				for (const auto& v : fs::directory_iterator(path.toUTF8()))
				{
					paths.push_back(detail::NormalizePath(Unicode::FromUTF8(fs::weakly_canonical(v.path()).string())));
				}
			}
		...
		bool ChangeCurrentDirectory(const FilePathView path)
		{
			...
			return (chdir(path.toUTF8().c_str()) == 0);
		}
		...
		bool Remove(const FilePathView path, const AllowUndo allowUndo)
		{
			...
			return detail::Linux_TrashFile(path.toUTF8().c_str());
		}
	}
}
