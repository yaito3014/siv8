...
namespace s3d
{
	namespace detail
	{
		...
		static GLFWcursor* CreateCursor(const Image& image, const Point hotSpot)
		{
			...
			cursorImage.pixels = (uint8*)image.data();
	...
	void CCursor::init()
	{
...

		{
			...
			m_state.update(clientPos.asPoint(), (clientPos / uiScaling).asPoint(), screenPos);
		}
	...
	void CCursor::updateHighTemporalResolutionCursorPos(Point rawClientPos) {
		// TODO
	}
