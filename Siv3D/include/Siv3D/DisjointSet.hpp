...
namespace s3d
{
	...
	public:
		...
		/// @param n 要素数
		/// @remark n が MaxSize を超える場合の動作は未定義です。
		[[nodiscard]]
		...
		/// @brief ノード i とノード k が同じグループに含まれているかを返します。
		/// @param i ノードのインデックス
		/// @param k ノードのインデックス
		/// @return ノード i とノード k が同じグループに含まれている場合 true, それ以外の場合は false
		[[nodiscard]]
		...
		/// @brief すべてのグループを列挙します。
		/// @return 各グループに属するノードのインデックスの一覧
		[[nodiscard]]
		Array<Array<index_type>> groups();
		...
		/// @brief 要素数を変更します。新しく追加された要素は単独のグループになります。
		/// @param n 新しい要素数
		/// @remark 現在の要素数より小さい n を指定した場合の動作は未定義です。
